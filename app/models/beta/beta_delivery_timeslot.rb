class BetaDeliveryTimeslot < ActiveRecord::Base
  ...
  def self.all_available_timeslots
    end_date = Date.parse("12/08/2016")
    ...
    for i in 0..num_days
      ...
      day_timeslots = BetaDeliveryTimeslot.where("time BETWEEN ? AND ?", day.beginning_of_day, day.end_of_day).order("time ASC").select(&:available?)
      timeslots[day] = day_timeslots if day_timeslots.size > 0
    end
