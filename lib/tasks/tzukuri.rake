...
namespace :tzukuri do
  ...
  namespace :diag do
    ...
    task :prod_battery_report => :environment do
      ...
      prod_users.each do |user|
        ...
        user.auth_tokens.each do |auth_token|
          ...
          write_report(out_str, "prod_report/#{start_time}/#{user.email}", "report_#{Time.now.strftime('%s')}_#{auth_token.diagnostics_sync_token}.csv")
        end
      end
    end
    ...
    desc "How long does the battery last for a sync token?"
    task :battery_readings => :environment do
      sync_tokens, dates = parse_args(ENV)
      abort("exiting, requires single token -- sync_tokens=[token]") if sync_tokens.blank? || sync_tokens.count != 1
